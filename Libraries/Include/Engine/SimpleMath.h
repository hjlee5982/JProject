...
namespace DirectX
{
    namespace SimpleMath
    {
        ...
        struct Vector2 : public XMFLOAT2
        {
            ...
            static void Transform(const Vector2& v, const matx& m, Vector2& result) noexcept;
            static Vector2 Transform(const Vector2& v, const matx& m) noexcept;
            static void Transform(_In_reads_(count) const Vector2* varray, size_t count, const matx& m, _Out_writes_(count) Vector2* resultArray) noexcept;

            static void Transform(const Vector2& v, const matx& m, Vector4& result) noexcept;
            static void Transform(_In_reads_(count) const Vector2* varray, size_t count, const matx& m, _Out_writes_(count) Vector4* resultArray) noexcept;

            static void TransformNormal(const Vector2& v, const matx& m, Vector2& result) noexcept;
            static Vector2 TransformNormal(const Vector2& v, const matx& m) noexcept;
            static void TransformNormal(_In_reads_(count) const Vector2* varray, size_t count, const matx& m, _Out_writes_(count) Vector2* resultArray) noexcept;
        ...
        struct Vector3 : public XMFLOAT3
        {
            ...
            static void TransformNormal(_In_reads_(count) const Vector3* varray, size_t count, const matx& m, _Out_writes_(count) Vector3* resultArray) noexcept;
        ...
        struct Vector4 : public XMFLOAT4
        {
            ...
            static void Transform(const Vector4& v, const matx& m, Vector4& result) noexcept;
            static Vector4 Transform(const Vector4& v, const matx& m) noexcept;
            static void Transform(_In_reads_(count) const Vector4* varray, size_t count, const matx& m, _Out_writes_(count) Vector4* resultArray) noexcept;
        ...
        struct matx : public XMFLOAT4X4
        {
            matx() noexcept
                : XMFLOAT4X4(1.f, 0, 0, 0,
                            ...
                            0, 0, 1.f, 0,
                            0, 0, 0, 1.f) {}
            constexpr matx(float m00, float m01, float m02, float m03,
                ...
                : XMFLOAT4X4(m00, m01, m02, m03,
                             ...
                             m20, m21, m22, m23,
                             m30, m31, m32, m33) {}
            explicit matx(const Vector3& r0, const Vector3& r1, const Vector3& r2) noexcept
                : XMFLOAT4X4(r0.x, r0.y, r0.z, 0,
                             r1.x, r1.y, r1.z, 0,
                             r2.x, r2.y, r2.z, 0,
                             0, 0, 0, 1.f) {}
            explicit matx(const Vector4& r0, const Vector4& r1, const Vector4& r2, const Vector4& r3) noexcept
                : XMFLOAT4X4(r0.x, r0.y, r0.z, r0.w,
                             r1.x, r1.y, r1.z, r1.w,
                             r2.x, r2.y, r2.z, r2.w,
                             r3.x, r3.y, r3.z, r3.w) {}
            matx(const XMFLOAT4X4& M) noexcept { memcpy(this, &M, sizeof(XMFLOAT4X4)); }
            matx(const XMFLOAT3X3& M) noexcept;
            matx(const XMFLOAT4X3& M) noexcept;

            explicit matx(_In_reads_(16) const float *pArray) noexcept : XMFLOAT4X4(pArray) {}
            matx(CXMMATRIX M) noexcept { XMStoreFloat4x4(this, M); }
            ...
            // Comparison operators
            bool operator == (const matx& M) const noexcept;
            bool operator != (const matx& M) const noexcept;

            // Assignment operators
            matx& operator= (const XMFLOAT3X3& M) noexcept;
            matx& operator= (const XMFLOAT4X3& M) noexcept;
            matx& operator+= (const matx& M) noexcept;
            matx& operator-= (const matx& M) noexcept;
            matx& operator*= (const matx& M) noexcept;
            matx& operator*= (float S) noexcept;
            matx& operator/= (float S) noexcept;
            ...
            // Unary operators
            matx operator+ () const noexcept { return *this; }
            matx operator- () const noexcept;
            ...
            matx Transpose() const noexcept;
            void Transpose(matx& result) const noexcept;

            matx Invert() const noexcept;
            void Invert(matx& result) const noexcept;

            float Determinant() const noexcept;

            // Static functions
            static matx CreateBillboard(
            ...
            static matx CreateConstrainedBillboard(
            ...
            static matx CreateTranslation(const Vector3& position) noexcept;
            static matx CreateTranslation(float x, float y, float z) noexcept;

            static matx CreateScale(const Vector3& scales) noexcept;
            static matx CreateScale(float xs, float ys, float zs) noexcept;
            static matx CreateScale(float scale) noexcept;

            static matx CreateRotationX(float radians) noexcept;
            static matx CreateRotationY(float radians) noexcept;
            static matx CreateRotationZ(float radians) noexcept;

            static matx CreateFromAxisAngle(const Vector3& axis, float angle) noexcept;

            static matx CreatePerspectiveFieldOfView(float fov, float aspectRatio, float nearPlane, float farPlane) noexcept;
            static matx CreatePerspective(float width, float height, float nearPlane, float farPlane) noexcept;
            static matx CreatePerspectiveOffCenter(float left, float right, float bottom, float top, float nearPlane, float farPlane) noexcept;
            static matx CreateOrthographic(float width, float height, float zNearPlane, float zFarPlane) noexcept;
            static matx CreateOrthographicOffCenter(float left, float right, float bottom, float top, float zNearPlane, float zFarPlane) noexcept;

            static matx CreateLookAt(const Vector3& position, const Vector3& target, const Vector3& up) noexcept;
            static matx CreateWorld(const Vector3& position, const Vector3& forward, const Vector3& up) noexcept;

            static matx CreateFromQuaternion(const Quaternion& quat) noexcept;

            static matx CreateFromYawPitchRoll(float yaw, float pitch, float roll) noexcept;

            static matx CreateShadow(const Vector3& lightDir, const Plane& plane) noexcept;

            static matx CreateReflection(const Plane& plane) noexcept;

            static void Lerp(const matx& M1, const matx& M2, float t, matx& result) noexcept;
            static matx Lerp(const matx& M1, const matx& M2, float t) noexcept;

            static void Transform(const matx& M, const Quaternion& rotation, matx& result) noexcept;
            static matx Transform(const matx& M, const Quaternion& rotation) noexcept;

            // Constants
            static const matx Identity;
        };

        // Binary operators
        matx operator+ (const matx& M1, const matx& M2) noexcept;
        matx operator- (const matx& M1, const matx& M2) noexcept;
        matx operator* (const matx& M1, const matx& M2) noexcept;
        matx operator* (const matx& M, float S) noexcept;
        matx operator/ (const matx& M, float S) noexcept;
        matx operator/ (const matx& M1, const matx& M2) noexcept;
            // Element-wise divide
        matx operator* (float S, const matx& M) noexcept;
        ...
        struct Plane : public XMFLOAT4
        {
            ...
            // Static functions
            static void Transform(const Plane& plane, const matx& M, Plane& result) noexcept;
            static Plane Transform(const Plane& plane, const matx& M) noexcept;
        ...
        public:
            ...
            Vector3 Project(const Vector3& p, const matx& proj, const matx& view, const matx& world) const noexcept;
            void Project(const Vector3& p, const matx& proj, const matx& view, const matx& world, Vector3& result) const noexcept;

            Vector3 Unproject(const Vector3& p, const matx& proj, const matx& view, const matx& world) const noexcept;
            void Unproject(const Vector3& p, const matx& proj, const matx& view, const matx& world, Vector3& result) const noexcept;
...
namespace std
{
    ...
    template<> struct less<DirectX::SimpleMath::matx>
    {
        bool operator()(const DirectX::SimpleMath::matx& M1, const DirectX::SimpleMath::matx& M2) const noexcept
        {
